...
- **v2 primitives — reuse before writing.** `src/components/common/` holds the design-system inventory: `Button`/`IconButton`, `Card`, `SegmentedControl`, `PillTabs`, `ToggleSwitch`, `Tag`/`CategoryTag`/`TickerChip`, `FilterChip`/`FilterPanel`/`FiltersButton`, `MobileFilterDropdown`/`MobileFilterFooter`, `Modal`, `DataGrid`/`ClickableRow`, `StatBlock`/`StatGrid`, `Donut`, `DottedBar`, `DeltaText`, `Avatar`/`EmojiAvatar`, `QueryGate`, `EmptyState`, `ChartTooltip`. Check there (and `utils/chartStyles.ts`, `utils/colors.ts`, `utils/tagTints.ts`) before hand-rolling a control, chip, grid or chart element; a missing case becomes a new size/variant on the primitive.
- **`clsx` only, no `tailwind-merge`.** Later classes do not override earlier ones, so a primitive's size/variant owns its height, padding, radius and border colour. Never pass `h-`/`px-`/`rounded-`/`border-<color>` through `className` to a primitive that already sets them — add a size or variant instead.
...
- **Semantic color tokens only.** New code uses the v2 tokens from `tailwind.config.ts` (`bg-bg`/`bg-surface`/`bg-surface-2`/`bg-raised`, `border-border`/`border-border-strong`, `text-text-1/2/3/muted/faint`, `text-accent`, `positive`/`negative`/`warning`) — never `neutral-*`, `bg-white`, `shadow-card`, `rounded-xl`/`rounded-full` on buttons or cards, or a teal literal (map through `brand-*`). The `html.dark` override wall in `styles/index.css` is a v1 bridge for pages not yet migrated; the wrap-up lane B PR removes it.
