...
WHERE -- :switch @cursor none datetime_gt datetime_lt amount_gt amount_lt default=none
  t.staged_for_llm = 0
  ...
  AND t.id NOT IN (SELECT transaction_id FROM transaction_tags) -- :flag @untagged
  ...
  AND cr.group_kind != 'TRANSFER' -- :flag @exclude_transfers
  AND cr.group_kind != 'INCOME' -- :flag @exclude_income
  AND TRUE -- :case @none
  AND (t.datetime > @cursor_datetime OR (t.datetime = @cursor_datetime AND t.id > @cursor_id)) -- :case @datetime_gt
  AND (t.datetime < @cursor_datetime OR (t.datetime = @cursor_datetime AND t.id < @cursor_id)) -- :case @datetime_lt
  AND (t.amount_cents > @cursor_amount OR (t.amount_cents = @cursor_amount AND t.id > @cursor_id)) -- :case @amount_gt
  AND (t.amount_cents < @cursor_amount OR (t.amount_cents = @cursor_amount AND t.id < @cursor_id)) -- :case @amount_lt
ORDER BY -- :switch @order amount_asc amount_desc datetime_asc datetime_desc id_asc id_desc default=id_asc
  t.amount_cents ASC, t.id ASC,   -- :case @amount_asc
  t.amount_cents DESC, t.id DESC, -- :case @amount_desc
  t.datetime ASC, t.id ASC,       -- :case @datetime_asc
  t.datetime DESC, t.id DESC,     -- :case @datetime_desc
  t.id ASC,                       -- :case @id_asc
  t.id DESC                       -- :case @id_desc
LIMIT @row_limit;
...
WHERE
  ...
  AND t.amount_cents <= @amount_max -- :if @amount_max
  -- :if @excluded_kinds
  AND cr.group_kind NOT IN (sqlc.slice('excluded_kinds'))
;
