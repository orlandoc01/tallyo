...
JOIN accounts a ON a.id = s.account_id
WHERE
  (s.synced_at >= @start_time AND s.synced_at < @end_time) -- :if @start_time @end_time
...
JOIN accounts a ON a.id = s.account_id
WHERE
  s.date < @after_date -- :if @after_date
  AND s.account_id = @account_id
...
-- Without an as-of window, closed accounts are dropped so their last row never stays current forever; with one
-- (the UTC bounds of a local day), a closed account counts while that day is on or before its final snapshot,
-- matching the historical series' forward fill.
SELECT
  sqlc.embed(a),
...
JOIN account_balance_daily_snapshots s ON s.account_id = a.id
  AND s.date = (
    ...
    WHERE s2.account_id = a.id
      AND (s2.synced_at < sqlc.narg('as_of_end') OR sqlc.narg('as_of_end') IS NULL)
  )
WHERE
  a.owner_id IN (sqlc.slice('owner_ids')) -- :if @owner_ids
  AND a.id IN (sqlc.slice('account_ids')) -- :if @account_ids
  AND a.is_hidden = 0
  AND (a.is_closed = 0 OR EXISTS (SELECT 1 FROM account_balance_daily_snapshots s3 WHERE s3.account_id = a.id AND s3.synced_at >= sqlc.narg('as_of_start')))
  AND a.type IN ('CREDIT', 'LOAN')
ORDER BY a.name;

-- name: ListCurrentAccountHoldings :many
-- Closed-account and as-of window semantics match ListLatestLiabilityAccountBalances above.
SELECT
  sqlc.embed(a),
  o.name AS owner_name,
  sqlc.embed(assets),
...
JOIN assets ON assets.id = h.asset_id
LEFT JOIN asset_analysis_reports r ON r.asset_id = assets.id -- :flag @classified_only
WHERE
  a.type NOT IN ('CREDIT', 'LOAN') -- :flag @exclude_liabilities
...
-- Each account's latest snapshot before the range is included as the anchor its early points forward-fill from.
...
WHERE
  ...
  AND (s.synced_at >= @start_time OR s.synced_at = (
    SELECT MAX(s2.synced_at)
    FROM account_balance_daily_snapshots s2
    WHERE s2.account_id = s.account_id
      AND s2.synced_at < @start_time
  ))
  AND a.is_hidden = 0
...
-- Anchors each account with its latest pre-range snapshot, like AccountBalanceSnapshotValues.
