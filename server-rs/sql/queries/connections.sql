...
WHERE
  c.source_table IN ('plaid_items', 'evm_wallets', 'simplefin_connections') -- :flag @supported_providers_only
  AND c.is_active = 1 -- :flag @active_only
  AND c.id IN (sqlc.slice('ids')) -- :if @ids
  AND c.source_table = @source_table -- :flag @source_lookup
  AND c.source_id = @source_id -- :flag @source_lookup
ORDER BY c.id;
