...
WHERE
  c.id IN (sqlc.slice('connection_ids')) -- :if @connection_ids
  AND c.is_active = 1 -- :flag @due_only
  AND (w.next_balance_sync_at IS NULL OR w.next_balance_sync_at <= @now) -- :flag @due_only
  AND EXISTS (SELECT 1 FROM accounts a WHERE a.connection_id = c.id AND a.manual = 0 AND a.type = 'CRYPTO_WALLET' AND a.is_closed = 0) -- :flag @due_only
ORDER BY
  w.created_at, -- :flag @due_only
  c.id;
