...
-- Used by internal/transactions/plaid's investments sync to find stored rows inside the re-fetched date window so ones Plaid no longer returns can be removed.
-- name: PlaidTransactionIDsInWindow :many
SELECT t.external_id
FROM transactions t
JOIN accounts a ON a.id = t.account_id
WHERE t.source = 'plaid'
  AND a.external_id IN (sqlc.slice('account_external_ids'))
