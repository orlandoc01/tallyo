...
WHERE
  ...
  AND r.id IN (SELECT rowid FROM rules_fts WHERE rules_fts MATCH @search_match) -- :if @search_match
...
WHERE
  (
    instr(lower(coalesce(t.merchant_name, t.original_name, '')), @merchant_pattern) > 0 -- :if @merchant_pattern
  ...
  -- :if @account_ids
  AND t.account_id IN (sqlc.slice('account_ids'))
;
