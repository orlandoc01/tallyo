...
WHERE
  (lh.asset_id IS NOT NULL OR a.user_created = 1) -- :flag @exclude_historical
  AND a.asset_type = @asset_type -- :if @asset_type
  ...
  AND (instr(lower(coalesce(a.name, '')), @like_name) > 0 OR instr(lower(a.identifier), @like_identifier) > 0) -- :flag @like_search
ORDER BY
  bm25(assets_fts) ASC, -- :if @fts_match
  lower(coalesce(a.name, '')) ASC, a.identifier ASC, -- :flag @order_name
  a.id ASC;
