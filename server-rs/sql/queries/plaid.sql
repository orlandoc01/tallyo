...
WHERE
  pc.id IN (sqlc.slice('ids')) -- :if @ids
GROUP BY pc.id
ORDER BY pc.id;
...
WHERE
  c.is_active = 1 -- :flag @active_only
...
JOIN owners o ON o.id = c.owner_id
WHERE
  c.is_active = 1 -- :flag @due_only
  AND pi.id = @id -- :if @id
  AND COALESCE(CASE CAST(@kind AS TEXT) WHEN 'sync' THEN pi.next_sync_at WHEN 'recurring' THEN pi.next_recurring_sync_at ELSE pi.next_balance_sync_at END, @now) <= @now -- :flag @due_only
  AND (
